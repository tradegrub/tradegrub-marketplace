# Directional Vortex Scanner

Enhanced vortex indicator with multi-period trend confirmation and signal quality filtering using numpy array operations. This trend-following indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
|-----------|---------|-------|-------------|
| Vortex Length | 14 | 5 - 50 | Controls vortex length sensitivity (int) |
| Confirmation Length | 21 | 10 - 60 | Controls confirmation length sensitivity (int) |
| Signal Threshold | 0.1 | 0.02 - 0.5 | Controls signal threshold sensitivity (float) |

## Signals

- **VI+**: Primary visual output plotted as a continuous line on the chart
- **VI-**: Primary visual output plotted as a continuous line on the chart
- **Vortex Diff**: Primary visual output plotted as a continuous line on the chart
- **Baseline** (1.0): Reference level for threshold-based decisions
- **Background shading**: Highlights active signal zones based on confirmed_bull.tolist()
- **Background shading**: Highlights active signal zones based on confirmed_bear.tolist()
