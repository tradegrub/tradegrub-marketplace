...
The tuple unpacking of `(squeeze_on, momentum)` from a single function call is a Python pattern that Pine cannot replicate. Pine requires separate variable assignments from the same function call using `plot` indices. The `squeeze_on[-2] and not squeeze_on[-1]` transition detection is natural array logic.
