...
The strategy uses vectorized boolean array operations and a full-history iteration loop that Pine Script cannot express:
...
The `&` operator performs element-wise AND across numpy boolean arrays, producing a single condition array that is True only where all three conditions align simultaneously. Pine Script processes one bar at a time and cannot create or manipulate arrays of boolean conditions this way.
