# Smooth Dual-Weight Crossover

Low-lag dual-weight moving average crossover using numpy cascaded smoothing with minimal phase distortion. This trend-following indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
|-----------|---------|-------|-------------|
| Fast Length | 8 | 3 - 30 | Controls fast length sensitivity (int) |
| Slow Length | 21 | 10 - 100 | Controls slow length sensitivity (int) |
| Phase Adjustment | 0.5 | 0.0 - 1.0 | Controls phase adjustment sensitivity (float) |

## Signals

- **Fast Smooth**: Primary visual output plotted as a continuous line on the chart
- **Slow Smooth**: Primary visual output plotted as a continuous line on the chart
