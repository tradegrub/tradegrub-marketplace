...
Python's `[-3]`, `[-2]`, `[-1]` indexing makes the three-bar relationship immediately readable. The compound boolean `bull_3bar` collapses six conditions into a single variable. Pine requires separate `close[2]`, `close[1]`, `close[0]` references with confusing forward-looking index numbering.
