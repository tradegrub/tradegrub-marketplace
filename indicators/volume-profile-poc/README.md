...
Price
 ...
 │           ╱     toward POC
 └──────────────────────────────── Time
    Volume   Price bounces between
    histogram   VAH and VAL
```
...
The volume profile construction uses Python for-loops with list operations and dynamic index arithmetic that would be extremely cumbersome in Pine Script's limited loop constructs:
...
The bidirectional Value Area expansion algorithm uses Python's full-featured `while` loop with conditional boundary checking, `int()` casting for bin indexing, and list methods like `.index(max(...))`. Pine Script's `for` loops have strict iteration limits and lack dynamic break conditions. The Python implementation handles edge cases (bin boundaries, uneven distributions) cleanly with standard conditional logic.
