...
Rolling Hurst exponent via R/S analysis to identify trending vs mean-reverting regimes and cycle pivot points. This cycle analysis indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
Core techniques include simple moving average, regression analysis, logarithmic transformation, iterative computation. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
| R/S Periods | 20 | 5 - 50 | Controls r/s periods sensitivity (int) |

## Signals

- **Hurst Exponent**: Primary visual output plotted as a continuous line on the chart
- **Cycle Pivot**: Discrete signal marker displayed at key points
- **Random Walk** (0.5): Reference level for threshold-based decisions
- **Trending** (1.0): Reference level for threshold-based decisions
- **Mean Reverting** (0.0): Reference level for threshold-based decisions
- **Background shading**: Highlights active signal zones based on (hurst < 0.5).tolist()
