...
The strategy leverages numpy's `np.maximum()` for element-wise cloud top computation across the entire price array in a single vectorized call, avoiding the bar-by-bar conditional logic required in Pine:
...
The `ta.ichimoku()` function returns all five Ichimoku lines in a single call with full-array computation, while Pine requires computing each component separately with redundant loops over the same data.
