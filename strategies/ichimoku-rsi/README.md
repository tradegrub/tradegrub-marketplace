...
Price
 ...
 |       /\                         /\
 |      /  \  Price above cloud    /  \  Price below cloud
 |     /    \      /\             /    \
 |    /      \    /  \           /      \
 |   /        \  /    \    /\  /        \
 |  /          \/      \  /  \/          \
...
Pine Script evaluates conditions one bar at a time, making complex multi-indicator logic slower and harder to debug. Here, `long_cond` and `short_cond` are complete boolean arrays covering every bar in the dataset, computed with numpy broadcasting. The `np.maximum` and `np.minimum` calls replace what would be nested `math.max()` calls on each bar in Pine Script.
