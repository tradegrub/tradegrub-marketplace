...
On-chart visual signal marker that identifies potential reversal points using multi-indicator scoring and labels them with strength classification via numpy and scipy. This reversal detection indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
Built-in technical functions used: `rsi, sma`. These provide the foundation for the indicator's calculations, computed efficiently across the full price history in a single pass.

Core techniques include exponential moving average, simple moving average, RSI, iterative computation. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
|-----------|---------|-------|-------------|
| Pivot Sensitivity | 5 | 2 - 15 | Controls pivot sensitivity sensitivity (int) |
| RSI Length | 14 | 5 - 30 | Controls rsi length sensitivity (int) |
| Volume Spike Mult | 1.5 | 1.0 - 3.0 | Controls volume spike mult sensitivity (float) |
| Label Cooldown | 10 | 5 - 30 | Controls label cooldown sensitivity (int) |

## Signals

- Primary output plotted on chart with threshold-based coloring
