# Volatility Estimator Suite

Four range-based volatility estimators (Parkinson, Garman-Klass, Rogers-Satchell, Yang-Zhang) computed with numpy for comparing realized volatility measures. This volatility indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
Core techniques include logarithmic transformation, iterative computation, extremum detection, high-low range analysis. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
|-----------|---------|-------|-------------|
| Lookback | 20 | 5 - 100 | Controls lookback sensitivity (int) |

## Signals

- **Parkinson**: Primary visual output plotted as a continuous line on the chart
- **Garman-Klass**: Primary visual output plotted as a continuous line on the chart
- **Rogers-Satchell**: Primary visual output plotted as a continuous line on the chart
- **Yang-Zhang**: Primary visual output plotted as a continuous line on the chart
