# Adaptive MACD Oscillator

MACD variant using adaptively smoothed moving averages for reduced noise and earlier signal detection via numpy filtering. This momentum indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
Core techniques include exponential moving average, simple moving average, iterative computation. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
|-----------|---------|-------|-------------|
| Fast Length | 12 | 5 - 30 | Controls fast length sensitivity (int) |
| Slow Length | 26 | 15 - 60 | Controls slow length sensitivity (int) |
| Signal Length | 9 | 3 - 20 | Controls signal length sensitivity (int) |

## Signals

- **MACD**: Primary visual output plotted as a continuous line on the chart
