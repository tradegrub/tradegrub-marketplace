...
The `np.roll()` function shifts the entire boolean array by one position to create a "previous bar" comparison, enabling flip detection without bar-by-bar loops. The `vol_confirmed` array is computed once across the full dataset. Pine has no equivalent to `np.roll()` and must use `[]` history references evaluated one bar at a time.
