...
Price
 ...
 +----------------------------------------- Time
   SHORT          EXIT       BUY LONG
  (cross under   (at basis)  (cross above
   upper)                     lower)
```
...
The tuple unpacking pattern `upper, basis, lower = ta.kc(...)` is a Python idiom that assigns three complete numpy arrays in one statement. Pine Script requires three separate variable assignments. The `[-1]` indexing for current-bar evaluation is clean and avoids the need for Pine Script's implicit series evaluation model.
