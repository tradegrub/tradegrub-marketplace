...
Price
 ...
 │       ════════════════════
 └──────────────────────────────── Time
    Strong Uptrend    Squeeze    New Trend
    (fanned, ordered)           (re-fans)
```
...
The list comprehension `[ta.sma(close, p) for p in periods]` would generate any number of SMAs from an arbitrary period list in a single line, something impossible in Pine Script where each SMA and its plot must be hardcoded. You could also compute ribbon width as `smas[0] - smas[-1]` for a quantitative spread metric, or use `all(smas[i] > smas[i+1] for i in range(len(smas)-1))` to detect perfectly ordered ribbons.
