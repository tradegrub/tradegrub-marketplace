...
Price
  ...
  |        ../        \..
  |      ./              \.
  |   ==/=Kalman=line======\===  Filtered trend (blue)
  |    /                    \
  | ../                      \..
  |.                            .  Lower confidence band
  |
  |  [Triangle up: bull reversal]
  |  [Triangle down: bear reversal]
  |
  |  Fill between bands shows uncertainty envelope
  +------------------------------------------------> Time
```
...
The Kalman filter requires matrix algebra that is fundamentally impossible in Pine Script:
...
Pine Script has no matrix types, no np.dot, no np.linalg.inv, and no way to maintain and propagate a covariance matrix. Any approximation would lose the mathematical guarantees that make the Kalman filter optimal.
