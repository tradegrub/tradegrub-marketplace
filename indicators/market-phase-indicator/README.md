...
Classifies market into accumulation, markup, distribution, and markdown phases using numpy volume-price analysis. This trend-following indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
Core techniques include simple moving average, iterative computation, volume analysis. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
|-----------|---------|-------|-------------|
| Phase Length | 20 | 10 - 50 | Controls phase length sensitivity (int) |

## Signals

- **Phase**: Primary visual output plotted as a continuous line on the chart
- **Markup** (3): Reference level for threshold-based decisions
- **Distribution** (2): Reference level for threshold-based decisions
- **Accumulation** (1): Reference level for threshold-based decisions
- **Markdown** (0): Reference level for threshold-based decisions
- **Background shading**: Highlights active signal zones based on (markup & valid).tolist()
- **Background shading**: Highlights active signal zones based on (distribution & valid).tolist()
- **Background shading**: Highlights active signal zones based on (accumulation & valid).tolist()
- **Background shading**: Highlights active signal zones based on (markdown & valid).tolist()
