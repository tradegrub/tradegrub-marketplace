...
The `ta.dmi()` function returns three arrays via tuple unpacking in a single call. The `&` operator performs element-wise AND across numpy boolean arrays, creating a composite condition that is True only where all three indicators agree. Pine must evaluate these conditions bar-by-bar and cannot pre-compute the full condition array.
