...
indicator("Session Edge Profiler", overlay=False)
...
for i in range(lookback, n):
    ...
    # Label extreme z-score alerts per session
    if np.abs(a_zscore[i]) > zscore_thresh:
        last_label_idx = i
        direction = "High" if a_zscore[i] > 0 else "Low"
        label.new(
            x=i, y=float(asian_edge[i]),
            text=f"Asian {direction}",
            style=label.style_label_down if a_zscore[i] > 0 else label.style_label_up,
            color="rgba(255,152,0,0.3)",
            textcolor="#FF9800",
            size="small"
        )
    elif np.abs(l_zscore[i]) > zscore_thresh:
        last_label_idx = i
        direction = "High" if l_zscore[i] > 0 else "Low"
        label.new(
            x=i, y=float(london_edge[i]),
            text=f"London {direction}",
            style=label.style_label_down if l_zscore[i] > 0 else label.style_label_up,
            color="rgba(33,150,243,0.3)",
            textcolor="#2196F3",
            size="small"
        )
    elif np.abs(ny_zscore[i]) > zscore_thresh:
        last_label_idx = i
        direction = "High" if ny_zscore[i] > 0 else "Low"
        label.new(
            x=i, y=float(ny_edge[i]),
            text=f"NY {direction}",
            style=label.style_label_down if ny_zscore[i] > 0 else label.style_label_up,
    ...
    if show_levels and i > 0:
        if asian_mask[i] > 0 and asian_mask[i - 1] == 0:
        ...
        elif london_mask[i] > 0 and london_mask[i - 1] == 0:
            if (i - last_label_idx) > cooldown // 2:
                label.new(
                    x=i, y=0.0,
                    text="London Open",
        ...
        elif ny_mask[i] > 0 and ny_mask[i - 1] == 0:
            if (i - last_label_idx) > cooldown // 2:
                label.new(
                    x=i, y=0.0,
                    text="NY Open",
