...
Range %                         Percentile
 ...
 │ ╱        ╲        ╲         0│              ╲
 └──────────────────────── Time  └──────────────── Time
   Expand   Contract  Expand      Breakout   Squeeze
                                  zone       zone
```
...
The `ta.percentrank` function computes a rolling percentile for every bar across the entire dataset in a single vectorized call. In Pine Script, `ta.percentrank` exists but cannot be chained with downstream array operations the way Python allows. You could extend this with `np.where(rng_percentile < 10, "extreme_squeeze", "normal")` for multi-level classification, or compute `np.diff(rng_percentile)` to detect the rate of change of the percentile for acceleration signals.
