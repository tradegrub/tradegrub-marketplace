...
Price
  ...
  |
  |  Cumulative Delta ---~~~~~----  (purple line)
  |
  |  [Whale bars marked with triangles below price]
  |  [Orange background on absorption zones]
  +------------------------------------------------> Time
```
...
The whale detection engine relies on rolling percentile calculations and histogram-based profiling that have no Pine Script equivalent:
...
Pine Script lacks np.percentile (only fixed quantile functions), np.digitize for histogram binning, and the ability to build arbitrary-resolution volume profiles from raw bar data. The conditional cumulative sum with directional signing would require dozens of lines of workaround code.
