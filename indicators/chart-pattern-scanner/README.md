...
```
  Price Series
      │
      v
  np.diff + np.sign ──> Turning Points
      │
      v
  Neighborhood Max/Min ──> Confirmed Swing Highs/Lows
      │                          │
      v                          v
  ┌────────────┐          ┌──────────────┐
  │ Peak Pairs │          │ Valley Pairs │
  │ Analysis   │          │ Analysis     │
  └─────┬──────┘          └──────┬───────┘
        │                        │
        v                        v
  Double Top     Double Bottom     Head & Shoulders
  (H1~H2)       (L1~L2)          (H1 < H2 > H3, H1~H3)
        │                        │
        └────────┬───────────────┘
                 v
  np.polyfit on Swing Series ──> Trendline Slopes
        │
        v
  Flat+Rising = Asc Triangle    Falling+Flat = Desc Triangle
  Converging = Wedge
        │
        v
  Composite Pattern Score ──> Plot + Alerts
```
...
The swing detection and pattern matching logic requires array-level operations that cannot be expressed in Pine Script:
