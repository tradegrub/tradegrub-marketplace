...
indicator("Supertrend Sensitivity Matrix", overlay=True)
...
# --- Rich annotations ---
last_trend_up_idx = -100
last_trend_down_idx = -100
last_consensus_idx = -100
ann_cooldown = 20

for i in range(1, n_bars):
    if show_labels and opt_change[i] > 0 and (i - last_trend_up_idx) > ann_cooldown:
        last_trend_up_idx = i
        label.new(
            x=i, y=float(opt_st_val[i]),
            text="Trend Up",
        ...
        if show_levels:
            line.new(x1=i, y1=float(opt_st_val[i]), x2=min(i + 20, n_bars - 1), y2=float(opt_st_val[i]),
                     color="#00e676", width=1, style=line.style_dashed)

    if show_labels and opt_change[i] < 0 and (i - last_trend_down_idx) > ann_cooldown:
        last_trend_down_idx = i
        label.new(
            x=i, y=float(opt_st_val[i]),
            text="Trend Down",
    ...
    if show_labels and i > 1 and (i - last_consensus_idx) > ann_cooldown * 2:
        if consensus_strength[i] > 0.9:
            last_consensus_idx = i
            direction_text = "Bull Consensus" if consensus_direction[i] > 0 else "Bear Consensus"
            c = "#00e676" if consensus_direction[i] > 0 else "#ef5350"
            label.new(
                x=i, y=float(opt_st_val[i]) * 1.001,
                text=direction_text,
