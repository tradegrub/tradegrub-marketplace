...
The tuple unpacking (`aroon_up, aroon_down = ta.aroon(...)`) and subsequent array arithmetic operates on the entire history simultaneously. In Pine Script, each of these would require separate variable declarations and the normalization math would execute bar-by-bar. Python makes it trivial to add a fourth component (e.g., `supertrend_norm`) by simply including it in the average and changing the divisor to 4.
