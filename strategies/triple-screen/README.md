...
The `&` operator performs element-wise AND across three numpy boolean arrays, producing a single condition array that is True only where all three screens align. Pine evaluates conditions bar-by-bar and cannot pre-compute the full condition array or chain multiple indicator pipelines into a single composite boolean. The `ta.change()` wrapper computes first-differences across the entire array in one call.
