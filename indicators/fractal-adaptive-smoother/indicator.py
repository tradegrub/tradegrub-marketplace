...
indicator("Fractal Adaptive Smoother", overlay=True)
...
for i in range(period, n):
    if show_labels:
        ...
        if trending[i] and not trending[i - 1] and (i - last_trend_idx) > cooldown:
            label.new(
        ...
        if choppy[i] and not choppy[i - 1] and (i - last_chop_idx) > cooldown:
            label.new(
                x=i, y=float(smoothed[i]),
                text="Choppy",
    ...
    if show_levels:
        # Price touching upper or lower band
