...
The entire KNN classification pipeline -- feature engineering, vectorized distance calculations, and weighted voting -- runs in pure numpy and would be impossible in Pine:
...
Pine has no matrix operations, no distance calculations across feature vectors, no argsort, and no way to store and query multi-dimensional analog databases. This indicator requires numpy's linear algebra and array manipulation at every step.
