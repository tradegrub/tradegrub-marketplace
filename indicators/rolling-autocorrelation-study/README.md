# Rolling Autocorrelation Study

Rolling autocorrelation analysis at multiple lags using numpy to detect serial dependence and mean reversion tendency. This statistical analysis indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
Core techniques include iterative computation, mean computation. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
|-----------|---------|-------|-------------|
| Rolling Window | 50 | 20 - 200 | Controls rolling window sensitivity (int) |

## Signals

- **Zero** (0): Reference level for threshold-based decisions
- **Positive** (0.2): Reference level for threshold-based decisions
- **Negative** (-0.2): Reference level for threshold-based decisions
- **Background shading**: Highlights active signal zones based on trending.tolist()
- **Background shading**: Highlights active signal zones based on mean_rev.tolist()
