# Smoothed Price Candles

EMA-smoothed OHLC candles that preserve actual close prices and actual high/low extremes for cleaner price action. This candlestick analysis indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
The indicator analyzes price data using candlestick analysis techniques to produce actionable signals.

Core techniques include exponential moving average, iterative computation, extremum detection, high-low range analysis. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
|-----------|---------|-------|-------------|
| Smooth Length | 5 | 2 - 20 | Controls smooth length sensitivity (int) |

## Signals

- **Smoothed Open**: Primary visual output plotted as a continuous line on the chart
- **Smoothed High**: Primary visual output plotted as a continuous line on the chart
- **Smoothed Low**: Primary visual output plotted as a continuous line on the chart
...
- Automate candlestick pattern recognition across all bars
- Score pattern reliability based on historical performance
- Filter patterns based on trend context and volume
- Identify multi-bar patterns that manual analysis might miss
