...
Price
 |  Bar1 (bearish)   Bar2 (bullish)
 |   ┌──┐  open        ┌──┐ close > Bar1 open
 |   │  │              │##│
 |   │  │              │##│
 |   │##│  close  open │##│
 |   └──┘        ───── └──┘
 |     |    Bar2 opens at/below Bar1 close
 |     |    Bar2 closes above Bar1 open
 |   Both bars have body ratio >= 0.5
 +──────────────────────────── Time
              🟢 BUY
     STOP: below lowest low of pattern
...
The `np.where(candle_range > 0, body / candle_range, 0)` expression handles the zero-division edge case across the entire array in a single vectorized call. Pine Script would require a per-bar ternary or `nz()` wrapper. The `np.abs()` function computes absolute values across the full price history without a loop.
