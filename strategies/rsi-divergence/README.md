...
The divergence detection uses rolling min/max lookups and multi-condition boolean logic that would be verbose in Pine:
...
Python's negative indexing (`[-1]`, `[-2]`) for bar references and compound boolean expressions with `and` operators make divergence logic readable. In Pine, detecting the "RSI made a higher low while price made a lower low" condition requires multiple `ta.valuewhen()` calls and nested `if` blocks.
