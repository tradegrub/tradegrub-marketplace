...
Volume-weighted simple moving average crossover with adaptive period selection based on numpy rolling volatility analysis. This trend-following indicator provides quantitative signals that can be applied to any liquid market across all timeframes.
...
Core techniques include simple moving average, iterative computation, mean computation, volume analysis. The computation processes all bars simultaneously using vectorized numpy operations, ensuring consistent results regardless of the dataset size.
...
|-----------|---------|-------|-------------|
| Fast Length | 10 | 3 - 30 | Controls fast length sensitivity (int) |
| Slow Length | 30 | 15 - 100 | Controls slow length sensitivity (int) |

## Signals

- **Fast VWMA**: Primary visual output plotted as a continuous line on the chart
- **Slow VWMA**: Primary visual output plotted as a continuous line on the chart
- **Cross Up**: Discrete signal marker displayed at key points
- **Cross Down**: Discrete signal marker displayed at key points
