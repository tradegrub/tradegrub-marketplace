...
```
  Session Windows (UTC)
  |----- Asian ------|---- London ----|------ NY ------|
  00:00          08:00           16:00           21:00

  Per Session:
  [Range Data] --> Rolling Window (stride_tricks)
       |
       v
  +------------------+     +------------------+
  | Percentile Ranks |     | Z-Score Analysis |
  | P25 / P50 / P75  |     | (val - mean)/std |
  +--------+---------+     +--------+---------+
           |                         |
           v                         v
  +------------------+     +------------------+
  | Histogram Scoring|     | Win Rate Tracker |
  | CDF-based rank   |     | direction pct    |
  +--------+---------+     +--------+---------+
           |                         |
           +----------+--------------+
                      |
